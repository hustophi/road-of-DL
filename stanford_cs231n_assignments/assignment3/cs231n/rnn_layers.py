...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    affine_out = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
    next_h = np.tanh(affine_out)
    cache = (x, prev_h, Wx, Wh, affine_out, next_h)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, affine_out, next_h = cache
    daffine_out = dnext_h * (1 - next_h ** 2)
    db = np.sum(daffine_out, axis= 0)
    dx = np.dot(daffine_out, Wx.T)
    dprev_h = np.dot(daffine_out, Wh.T)
    dWx = np.dot(x.T, daffine_out)
    dWh = np.dot(prev_h.T, daffine_out)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dnext_h = dh[:, t, :] + dprev_h        #先计算每一时间步的总梯度
        dx[:, t, :], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, cache[t])   #再反向传播
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dprev_h
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache
    dW = np.zeros_like(W)
    np.add.at(dW, x, dout)           #在指定的位置上加上指定的值,eg:dW为返回值，x为索引矩阵，dout为指定的值
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a_i, a_f, a_o, a_g = np.hsplit(A, 4)#A[:, :H], A[:, H:2*H], A[:, 2*H:3*H], A[:, 3*H:]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dh_dc = dnext_h * o * (1 - tanh_c**2)
    dprev_c = (dh_dc + dnext_c) * f
    di = (dh_dc + dnext_c) * g
    df = (dh_dc + dnext_c) * prev_c
    do = dnext_h * tanh_c
    dg = (dh_dc + dnext_c) * i
    dA = np.hstack([di * i * (1 - i), df * f * (1 - f), do * o * (1 - o), dg * (1 - g**2)]) #(N, 4H)
    dWx = np.dot(x.T, dA)
    dx = np.dot(dA, Wx.T)
