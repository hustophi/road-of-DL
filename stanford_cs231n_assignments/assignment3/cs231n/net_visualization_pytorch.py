...
def make_fooling_image(X, target_y, model):
    ...
    while torch.argmax(scores, dim= 1) != target_y:
        score = scores[0, target_y]
        score.backward()
        with torch.no_grad():
            grad = X_fooling.grad
            grad = learning_rate * grad / torch.linalg.norm(grad)
            X_fooling += grad
        scores = model(X_fooling)
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    scores = model(img)
    score = scores[:, target_y] - l2_reg * torch.sum(img ** 2)
    score.backward()
    with torch.no_grad():
        img += learning_rate * img.grad     
        img.grad.zero_()                #在默认情况下，PyTorch会累积梯度,我们需要清除之前的值
