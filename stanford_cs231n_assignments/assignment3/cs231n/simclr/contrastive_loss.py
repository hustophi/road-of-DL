...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        exp_sum_k, exp_sum_k_N = 0, 0
        for i in range(2 * N):
            if i != k:
                exp_sum_k += torch.exp(sim(z_k, out[i]) / tau)
            if i != k+N:
                exp_sum_k_N += torch.exp(sim(z_k_N, out[i]) / tau)
        left_right = - torch.log(torch.exp(sim(z_k, z_k_N) / tau) / exp_sum_k)
        right_left = - torch.log(torch.exp(sim(z_k_N, z_k) / tau) / exp_sum_k_N)
        total_loss += left_right + right_left
...
def sim_positive_pairs(out_left, out_right):
    ...
    left_norm = torch.linalg.norm(out_left, dim= 1, keepdim= True)   #(N,1)
    right_norm = torch.linalg.norm(out_right, dim= 1, keepdim= True)
    pos_pairs = torch.sum(out_left * out_right, dim= 1, keepdim= True) / (left_norm * right_norm)
...
def compute_sim_matrix(out):
    ...
    normed_out = out / torch.linalg.norm(out, dim= 1, keepdim= True)
    sim_matrix = torch.matmul(normed_out, normed_out.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = exponential.masked_select(mask).view(2 * N, -1)  # [2*N, 2*N]
