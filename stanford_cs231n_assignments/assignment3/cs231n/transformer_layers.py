...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.dropout(x+ self.get_buffer('pe')[:, :S, :])
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.num_heads
        Q = self.query(query).reshape(N, S, H, D//H).transpose(1, 2)   #(N, H, S, E/H)
        K = self.key(key).reshape(N, T, H, D//H).transpose(1, 2)     #(N, H, T, E/H)
        V = self.value(value).reshape(N, T, H, D//H).transpose(1, 2)  #(N, H, T, E/H)
        attention = torch.matmul(Q, K.transpose(2, 3)) / math.sqrt(D//H)   #(N, H, S, T)
        if attn_mask is not None:
            attention = attention.masked_fill(attn_mask == 0, -float('inf'))
        attention = torch.softmax(attention, dim= 3)
        Y = self.dropout(torch.matmul(attention, V))    #(N, H, S, E/H)
        output = self.proj(Y.transpose(1, 2).reshape(N, S, -1))
        '''
        H = self.num_heads
