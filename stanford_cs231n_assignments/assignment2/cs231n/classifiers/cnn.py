...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params['W3'] = np.random.randn(hidden, num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        loss, ds = softmax(scores, y)
        loss += 0.5 * self.reg * (np.sum(W1**2) + np.sum(b1**2) + np.sum(W2**2) + np.sum(b2**2) + np.sum(W3**2) + np.sum(b3**2))
        dout, grads['W3'], grads['b3'] = affine_backward(ds, aff_cache)
        grads['W3'] += self.reg * W3
        grads['b3'] += self.reg * b3
        dout, grads['W2'], grads['b2'] = affine_relu_backward(dout, hidden_cache)
        grads['W2'] += self.reg * W2
        grads['b3'] += self.reg * b2
