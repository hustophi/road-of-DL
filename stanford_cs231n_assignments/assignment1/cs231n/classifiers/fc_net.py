...
class TwoLayerNet(object):
    ...
    def __init__(
        ...
        self.params['W1'], self.params['W2'] = np.random.randn(input_dim, hidden_dim) * weight_scale, \
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss_noreg, dx = softmax_loss(scores, y)
        loss = loss_noreg + 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))    #注意无需对bias正则
